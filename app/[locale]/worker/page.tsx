...
export default function WorkerHomePage() {
  ...
  async function submitStart() {
    if (!drawing?.currentOperation) return;
    setSubmitting(true);
    setError("");
    setMessage("");
  ...
  async function submitReport() {
    ...
    const goodQty = Math.max(0, drawing.currentOperation.maxGoodCanReport - parsedScrap);
    if (goodQty <= 0 && parsedScrap <= 0) {
      setError("当前无可报工数量。");
      return;
    }
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <button
                type="button"
                onClick={submitStart}
                disabled={submitting || !drawing.currentOperation}
              ...
              <button
                ...
                disabled={submitting || drawing.status === "OUTSOURCING"}
